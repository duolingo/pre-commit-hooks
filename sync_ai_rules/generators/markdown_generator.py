    ...
    def generate(self, rules: Dict[str, List[RuleMetadata]], config: Dict[str, Any]) -> str:
        ...
        lines = [
            ...
            🛑 STOP! READ THIS FIRST 🛑
             CRITICAL: You MUST complete this checklist BEFORE starting ANY work.**
        
            **Do NOT search code or begin implementation until you have:**

            1. **Scanned ALL Development Rules below** - Read every rule description to find matches
            2. **Read the ENTIRE matching rule file(s)** - Not just the summary, the FULL file
            3. **Confirmed which process to follow** - Know exactly what steps the rule requires
            "",
        ]
