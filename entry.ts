...
/** Check main.tf for duplicated subservice names and throw an error
 * if any are found. */
const findDuplicatedSubservices = (source: string, data: string) => {
  if (basename(source) == "main.tf") {
    toJSON(data).then((jsonObj: any) => {
      let subservices = new Set<String>();
      let repeatedSubservices = new Set<String>();
      if ("module" in jsonObj) {
        for (const module of jsonObj["module"]) {
          if (Object.keys(module).length > 0) {
            const module_name = Object.keys(module)[0];
            const module_array = module[module_name];
            if (module_array.length > 0) {
              const module_elements_object = module_array[0];
              if ("subservice" in module_elements_object) {
                const subservice = module_elements_object["subservice"];
                if (subservices.has(subservice)) {
                  repeatedSubservices.add(subservice);
                } else {
      ...
      if (repeatedSubservices.size != 0) {
        throw new Error("Error! Repeated subservice. Please make sure that " +
          "none of your modules in main.tf have the same subservice name.\n" +
          `Repeated subservices: ${Array.from(repeatedSubservices.values()).join(", ")}`)
      }
    });
  }
}
  ...
  [HookName.TerraformVld]: createLockableHook({
    action: sources => Promise.all(
      sources.map(source => transformFile(source, data => {
        findDuplicatedSubservices(source, data);
        return data;
      }))
    ),
    include: /\.tf$/,
